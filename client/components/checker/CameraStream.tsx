...
interface CameraPanelProps {
  ...
  // Remove sendMessage and isWebSocketConnected props - we'll handle WebSocket directly
}
...
export default function CameraPanel({
  ...
  const startStreaming = useCallback(async () => {
    if (appStep !== "INTERACTIVE_SESSION") return;
    ...
    try {
      ...
      webSocket.current.onopen = () => {
        ...
        // Start sending media - EXACTLY like working code
  ...
  const setupUserAudioStreaming = useCallback(() => {
    ...
    userAudioProcessor.current.onaudioprocess = (event: AudioProcessingEvent) => {
      if (webSocket.current?.readyState === WebSocket.OPEN) {
        ...
        console.log('🎵 Sending audio chunk directly - Size:', base64Data.length);
      ...
      case 'FAILED':
        return (
          <div className="w-full aspect-video bg-red-100 rounded-lg flex flex-col items-center justify-center text-center p-4">
            ...
            <button onClick={onReset} className="px-6 py-2 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 transition-colors">
              Try Again
            </button>
          </div>
        );
      ...
      default:
        return (
          <div className="w-full aspect-video bg-gray-900 rounded-lg flex flex-col items-center justify-center text-center p-4">
            ...
            <button onClick={onStartCamera} className="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors">
              Start Camera
            </button>
          </div>
        );
